...
public class YUVTools {
    ...
    public static int[] planesToColors(ImageProxy.PlaneProxy[] planeProxies, int height) {
        ByteBuffer yPlane = planeProxies[0].getBuffer();
        ByteBuffer uPlane = planeProxies[1].getBuffer();
        ByteBuffer vPlane = planeProxies[2].getBuffer();

        int bufferIndex = 0;
        final int total = yPlane.capacity();
        final int uvCapacity = uPlane.capacity();
        final int width = planeProxies[0].getRowStride();
    ...
    public static ImageBytes getBytesFromImage(Image image) {
        ...
        return getBytesFromByteBuffers(b0, b1, b2, p0.getRowStride(), p1.getRowStride(), p2.getRowStride());
    }

    public static ImageBytes getBytesFromImage(ImageProxy imageProxy) {
        final ImageProxy.PlaneProxy[] planes = imageProxy.getPlanes();

        ImageProxy.PlaneProxy p0 = planes[0];
        ImageProxy.PlaneProxy p1 = planes[1];
        ImageProxy.PlaneProxy p2 = planes[2];
    ...
    private static ImageBytes getBytesFromByteBuffers(ByteBuffer b0, ByteBuffer b1, ByteBuffer b2, int b0RowStride, int b1RowStride, int b2RowStride) {
        int r0 = b0.remaining();
        int r1 = b1.remaining();
        int r2 = b2.remaining();

        int h0 = r0 / b0RowStride;
        if (r0 % b0RowStride > 0) h0++;
        int h1 = r1 / b1RowStride;
        if (r1 % b1RowStride > 1) h1++;
        int h2 = r2 / b2RowStride;
        if (r2 % b2RowStride > 2) h2++;

        int y = b0RowStride * h0;
        int u = b1RowStride * h1;
        int v = b2RowStride * h2;
        ...
        return new ImageBytes(bytes, b0RowStride, h0);
    }
